...
@Schema(
    ...
    description = "Polls by running an inline .NET script in a container (default image mcr.microsoft.com/dotnet/sdk:10.0) and emits when exitCondition matches. Supports edge mode to emit only on transitions and polls every 60s by default. Accepts 'exit N' or a regex (fallback substring) matched against emitted vars and failure logs."
)
@Plugin(
    examples = {
        @Example(
