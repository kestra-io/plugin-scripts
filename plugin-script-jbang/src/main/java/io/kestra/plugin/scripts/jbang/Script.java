        ...
        return commands
            ...
            .withCommands(
                Property.ofValue(
                    List.of(
                        String.join(
                            " ", "jbang", runContext.render(quiet).as(Boolean.class).orElseThrow() ? "--quiet" : "",
                            commands.getTaskRunner().toAbsolutePath(runContext, commands, relativeScriptPath.toString(), os)
                        )
                    )
                )
            )
