...
@NoArgsConstructor
@Schema(
    title = "Trigger a flow when PowerShell commands match a condition",
    description = """
        Polls by running PowerShell commands in a script container (default image ghcr.io/kestra-io/powershell:latest) and emits when exitCondition matches. \
    ...
    examples = {
        @Example(
            ...
            code = """
                ...
                triggers:
                  - id: commands_failure
                    type: io.kestra.plugin.scripts.powershell.CommandsTrigger
                    interval: PT60S
...
public class CommandsTrigger extends AbstractTrigger
    ...
    @Schema(
        ...
        description = """
            Container image used by the underlying Commands task to run PowerShell commands.
            Defaults to 'ghcr.io/kestra-io/powershell:latest'.
            """
    )
    @Builder.Default
    ...
    @Schema(
        title = "PowerShell commands to execute",
        description = "Commands executed on each poll (same semantics as the PowerShell Commands task)."
    )
    @NotNull
    ...
    public static class Output implements io.kestra.core.models.tasks.Output {
        ...
        @Schema(
            ...
            description = "Exit code returned by the PowerShell process (may be null if not available)."
        )
