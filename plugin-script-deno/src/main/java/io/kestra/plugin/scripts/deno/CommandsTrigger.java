...
@NoArgsConstructor
@Schema(
    title = "Trigger a flow when Deno commands match a condition",
    description = "Polls and triggers a flow by running Deno commands within a script container."
)
@Plugin(
    examples = {
        @Example(
            ...
            code = """
                ...
                triggers:
                  - id: commands_failure
                    type: io.kestra.plugin.scripts.deno.CommandsTrigger
                    interval: PT10S
...
public class CommandsTrigger extends AbstractTrigger
    ...
    @Schema(
        ...
        description = """
            Container image used by the underlying Commands task to run Deno commands.
            Defaults to 'denoland/deno'.
            """
    )
    @Builder.Default
    ...
    @Schema(
        title = "Deno commands to execute",
        description = "Commands executed on each poll (same semantics as the Deno Commands task)."
    )
    @NotNull
    ...
    public static class Output implements io.kestra.core.models.tasks.Output {
        ...
        @Schema(
            ...
            description = "Exit code returned by the Deno process (may be null if not available)."
        )
