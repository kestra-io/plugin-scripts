...
@NoArgsConstructor
@Schema(
    title = "Trigger on Bun script condition",
    description = "Polls by running an inline Bun script in a container (default image oven/bun) and emits when exitCondition matches. Supports edge mode to emit only on transitions and polls every 60s by default. Accepts 'exit N' or a regex (fallback substring) matched against emitted vars and failure logs."
)
@Plugin(
    examples = {
        @Example(
            ...
            code = """
                ...
                triggers:
                  - id: script_failure
                    type: io.kestra.plugin.scripts.bun.ScriptTrigger
                    interval: PT10S
...
public class ScriptTrigger extends AbstractTrigger
    ...
    @Schema(
        ...
        description = """
            Image used by the Script task to run the inline Bun script; defaults to 'oven/bun'.
            Provide an image that includes the Bun runtime.
            """
    )
    @Builder.Default
    ...
    @Schema(
        title = "Inline Bun script",
        description = """
            Multi-line script executed on each poll, with the same semantics as the Bun Script task.
            """
    )
    @NotNull
