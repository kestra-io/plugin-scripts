...
@Plugin(
    examples = {
        @Example(
            title = "Trigger when Python command fails.",
            full = true,
            code = """
...
public class CommandsTrigger extends AbstractTrigger
    ...
    @Schema(
        ...
        description = "Container image used by the underlying Commands task."
    )
    @Builder.Default
    ...
    @Schema(
        ...
        description = """
            If true, the trigger emits only on transition from non-match to match.
            """
    )
    @Builder.Default
