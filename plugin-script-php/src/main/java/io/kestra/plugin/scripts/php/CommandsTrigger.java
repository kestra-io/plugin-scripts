...
@NoArgsConstructor
@Schema(
    title = "Trigger a flow when PHP commands match a condition",
    description = "Polls and triggers a flow by running system commands in a PHP container."
)
@Plugin(
    examples = {
        @Example(
            ...
            code = """
                ...
                triggers:
                  - id: commands_failure
                    type: io.kestra.plugin.scripts.php.CommandsTrigger
                    interval: PT10S
...
public class CommandsTrigger extends AbstractTrigger
    ...
    @Schema(
        ...
        description = """
            Container image used by the underlying Commands task to run PHP commands.
            Defaults to 'php'.
            """
    )
    @Builder.Default
    ...
    @Schema(
        title = "PHP commands to execute",
        description = "Commands executed on each poll (same semantics as the PHP Commands task)."
    )
    @NotNull
