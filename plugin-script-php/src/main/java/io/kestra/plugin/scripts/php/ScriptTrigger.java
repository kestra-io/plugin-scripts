...
@NoArgsConstructor
@Schema(
    title = "Trigger on PHP script condition",
    description = "Polls by running an inline PHP script in a container (default image php) and emits when exitCondition matches. Supports edge mode to emit only on transitions and polls every 60s by default. Accepts 'exit N' or a regex (fallback substring) matched against emitted vars and failure logs; run untrusted scripts only in trusted images."
)
@Plugin(
    examples = {
        @Example(
            ...
            code = """
                ...
                triggers:
                  - id: script_failure
                    type: io.kestra.plugin.scripts.php.ScriptTrigger
                    interval: PT10S
...
public class ScriptTrigger extends AbstractTrigger
    ...
    @Schema(
        ...
        description = """
            Image used by the Script task to run the inline PHP script; defaults to 'php'.
            Provide an image that includes the needed PHP runtime and extensions.
            """
    )
    @Builder.Default
    ...
    @Schema(
        ...
        description = """
            Multi-line script executed on each poll, with the same semantics as the PHP Script task.
            """
    )
    @NotNull
    ...
    public static class Output implements io.kestra.core.models.tasks.Output {
        ...
        @Schema(
            ...
            description = "Exit code returned by the PHP process (may be null if not available)."
        )
