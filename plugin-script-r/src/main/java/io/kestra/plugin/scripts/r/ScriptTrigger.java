...
@Getter
@NoArgsConstructor
@Schema(title = "Trigger a flow when an R script matches a condition", description = "Polls by running an R script and starts the flow when its output matches the condition.")
@Plugin(
    examples = {
        @Example(
...
public class ScriptTrigger extends AbstractTrigger
    ...
    @Schema(
        ...
        description = "Image used by the Script task to run the inline R script; defaults to 'r-base'."
    )
    @Builder.Default
